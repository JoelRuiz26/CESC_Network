...
# ==========================================================
# HEATMAP de sRGES (solo fármacos significativos) A7 vs A9
# ==========================================================
# ==========================================================# ==========================================================
# Heatmap TOP10 (Shared, A7, A9) con A7/A9 en eje X y barra de categoría
# ==========================================================
suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(pheatmap); library(stringr)
})

dir.create("6_2_4_PLOTS", showWarnings = FALSE)

thr <- -0.25

# 1) Tablas por clado
tab_A7 <- result_A7 %>%
  dplyr::select(pert_iname, A7 = sRGES) %>%
  dplyr::filter(A7 <= thr)

tab_A9 <- result_A9 %>%
  dplyr::select(pert_iname, A9 = sRGES) %>%
  dplyr::filter(A9 <= thr)

# 2) Unir y clasificar
join_all <- dplyr::full_join(tab_A7, tab_A9, by = "pert_iname") %>%
  dplyr::mutate(Categoria = dplyr::case_when(
    !is.na(A7) & !is.na(A9) ~ "Shared",
    !is.na(A7) &  is.na(A9) ~ "clade_A7",
    is.na(A7)  & !is.na(A9) ~ "clade_A9"
  )) %>%
  dplyr::filter(!is.na(Categoria))

# 3) Top-10 por categoría
top_shared <- join_all %>%
  dplyr::filter(Categoria == "Shared") %>%
  dplyr::rowwise() %>% dplyr::mutate(rank_val = min(c(A7, A9), na.rm = TRUE)) %>%
  dplyr::ungroup() %>% dplyr::arrange(rank_val) %>% dplyr::slice_head(n = 10) %>%
  dplyr::select(-rank_val)

top_A7 <- join_all %>% dplyr::filter(Categoria == "clade_A7") %>% dplyr::arrange(A7) %>% dplyr::slice_head(n = 10)
top_A9 <- join_all %>% dplyr::filter(Categoria == "clade_A9") %>% dplyr::arrange(A9) %>% dplyr::slice_head(n = 10)

sel <- dplyr::bind_rows(top_shared, top_A7, top_A9)

# 4) Matriz (filas=fármacos; columnas=A7/A9)
mat <- sel %>%
  dplyr::select(pert_iname, A7, A9) %>%
  tibble::column_to_rownames("pert_iname") %>%
  as.matrix()

# Orden: Shared → A7 → A9; dentro por efecto más negativo
ord_cat <- match(sel$Categoria, c("Shared","clade_A7","clade_A9"))
ord_pot <- apply(mat, 1, function(x) min(x, na.rm = TRUE))
row_order <- order(ord_cat, ord_pot)
mat <- mat[row_order, , drop = FALSE]

row_ann <- sel %>%
  dplyr::slice(row_order) %>%
  dplyr::select(Categoria) %>%
  as.data.frame()
rownames(row_ann) <- rownames(mat)

# 5) Escala y colores (más negativo = rojo)
rng <- range(mat, na.rm = TRUE); if (rng[2] > 0) rng[2] <- 0
breaks <- seq(rng[1], 0, length.out = 101)
cols <- colorRampPalette(c("#B2182B","#EF8A62","#FDDAD3","#FFFFFF","#D1E5F0","#67A9CF","#2166AC"))(100)
ann_cols <- list(Categoria = c(Shared="#33A02C", clade_A7="#1F78B4", clade_A9="#E31A1C"))

# 6) Etiquetas legibles y columnas A7/A9 visibles
row_labels <- stringr::str_trunc(rownames(mat), 42)
col_labels <- c("A7","A9")

# 7) Dispositivos con tamaño adecuado (no se corta)
png("6_2_4_PLOTS/Drug_sRGES_heatmap_TOP10_labeled.png",
    width = 900, height = max(700, nrow(mat) * 26), res = 150)
pheatmap(
  mat,
  color = cols, breaks = breaks,
  cluster_rows = FALSE, cluster_cols = FALSE,
  labels_row = row_labels,
  labels_col = col_labels,             # <-- A7 y A9 en eje X
  annotation_row = row_ann,            # <-- barra de categoría al lado
  annotation_colors = ann_cols,
  na_col = "grey92",
  main = "Top-10 Shared / A7 / A9 (sRGES ≤ −0.25)",
  fontsize_row = 11, fontsize_col = 12, fontsize = 10,
  cellwidth = 50,                       # columnas más anchas para ver A7/A9
  cellheight = 20,                      # ajusta si los nombres aún se cortan
  border_color = NA,
  gaps_row = c(
    max(which(row_ann$Categoria == "Shared")),
    max(which(row_ann$Categoria %in% c("Shared","clade_A7")))
  )
)
dev.off()

pdf("6_2_4_PLOTS/Drug_sRGES_heatmap_TOP10_labeled.pdf",
    width = 7.2, height = max(6.5, nrow(mat) * 0.33))
pheatmap(
  mat,
  color = cols, breaks = breaks,
  cluster_rows = FALSE, cluster_cols = FALSE,
  ...
  na_col = "grey92",
  main = "Top-10 Shared / A7 / A9 (sRGES ≤ −0.25)",
  fontsize_row = 10.5, fontsize_col = 12, fontsize = 10,
  cellwidth = 42, cellheight = 18,
  border_color = NA,
  gaps_row = c(
    max(which(row_ann$Categoria == "Shared")),
    max(which(row_ann$Categoria %in% c("Shared","clade_A7")))
