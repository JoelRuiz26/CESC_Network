# =====================
# In Silico Validation Plots (read-only; exact same outputs)
# =====================
...
# We rely on your fixed format:
#   line: "IC50 cortest" or "AUC cortest" (exact)
#   nearby: "t = ..., df = ..., p-value = <number>"
#   then a "sample estimates:" block with a single numeric "cor"
.parse_cor_block <- function(lines, start_idx){
  rng <- seq.int(start_idx, min(length(lines), start_idx + 60))
  
  # p-value line (simple, robust substring take)
  t_rel <- grep("p[- ]?value\\s*=", lines[rng], ignore.case = TRUE)
  if (!length(t_rel)) return(NULL)
  tline <- lines[rng[t_rel[1]]]
  # Take literal text after the first "=" up to end-of-line or comma/newline
  p_raw <- sub(".*p[- ]?value\\s*=\\s*([^,\\n]+).*", "\\1", tline, ignore.case = TRUE)
  p_num <- suppressWarnings(as.numeric(gsub("^\\s*[<>]=?\\s*", "", p_raw)))
  
  # r from the 'sample estimates' block (next few lines contain 'cor')
  est_rel <- grep("sample estimates", lines[rng], ignore.case = TRUE)
  r_val <- NA_real_
  if (length(est_rel)){
    look <- seq.int(rng[est_rel[1]] + 1, min(length(lines), rng[est_rel[1]] + 5))
    for (kk in look){
      # grab first numeric token (covers 0.123, -0.12, 1.2e-3, NA/NaN)
      got <- regmatches(lines[kk], regexpr("[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?|NaN|NA",
                                           lines[kk], perl = TRUE))
...
# ---- (Facets) Extract the already-drawn smooth line from the HTML ----
# We do NOT recompute. We only overlay the same line the widget used.
...
# ---- Collect r,p labels per cell line (from TXT; HTML fallback removed) ----
...
plot_facets_ic50 <- function(tbl, title_prefix, metrics_tbl, smooth_df, fda_set){
  ...
  # label only FDA + StronglyPredicted == "Yes" (exactly as before)
  labs_df <- tbl %>%
...
plot_global_ic50 <- function(tbl, title_prefix){
  ...
  labs_df <- tbl %>%
    filter(fda_flag == "FDA", StronglyPredicted == "Yes",
           is.finite(sRGES), is.finite(medIC50)) %>%
    group_by(pert_iname) %>%
    summarise(sRGES = median(sRGES, na.rm = TRUE),
              y     = median(log10(medIC50), na.rm = TRUE),
  ...
  ggplot(tbl, aes(x = sRGES, y = log10(medIC50))) +
    ...
    { if (nrow(labs_df))
      ggrepel::geom_text_repel(
        ...
        box.padding = 0.6, point.padding = 0.35, nudge_y = 0.2, direction = "y")
    } +
  ...
  ggplot(tbl, aes(x = sRGES, y = medauc)) +
    ...
    { if (nrow(labs_df))
      ggrepel::geom_text_repel(
        ...
        aes(x = sRGES, y = y, label = pert_iname),
        size = 2.6, segment.color = "grey80", max.overlaps = Inf,
        box.padding = 0.6, point.padding = 0.35, nudge_y = 0.02, direction = "y")
    } +
    scale_shape_manual(values = c(FDA = 17, Other = 16), name = "FDA status") +
    labs(title = sprintf("%s", title_prefix),
         y = "AUC", x = "sRGES") +
...
# ---- Build plots ----
data_A7_ic50_fda <- add_fda_flag(prep_clade_data(base_dir_A7)$ic50, fda_set_A7)
data_A7_auc_fda  <- add_fda_flag(prep_clade_data(base_dir_A7)$auc,  fda_set_A7)
data_A9_ic50_fda <- add_fda_flag(prep_clade_data(base_dir_A9)$ic50, fda_set_A9)
data_A9_auc_fda  <- add_fda_flag(prep_clade_data(base_dir_A9)$auc,  fda_set_A9)
...
# Facets
p_ic50_A7 <- plot_facets_ic50(data_A7_ic50_fda, "HPV-A7", metrics_A7, smooth_A7_ic50, fda_set_A7)
p_auc_A7  <- plot_facets_auc (data_A7_auc_fda,  "HPV-A7", metrics_A7, smooth_A7_auc,  fda_set_A7)
p_ic50_A9 <- plot_facets_ic50(data_A9_ic50_fda, "HPV-A9", metrics_A9, smooth_A9_ic50, fda_set_A9)
p_auc_A9  <- plot_facets_auc (data_A9_auc_fda,  "HPV-A9", metrics_A9, smooth_A9_auc,  fda_set_A9)

# Globals
p_ic50_A7_glob  <- plot_global_ic50(data_A7_ic50_fda, "HPV-A7")
p_auc_A7_glob   <- plot_global_auc (data_A7_auc_fda,  "HPV-A7")
p_ic50_A9_glob  <- plot_global_ic50(data_A9_ic50_fda, "HPV-A9")
p_auc_A9_glob   <- plot_global_auc (data_A9_auc_fda,  "HPV-A9")
...
save_plot <- function(p, name, w = 12, h = 7, dpi = 600, dir = plots_out_dir) {
  ...
  ggsave(file.path(dir, paste0(name, ".png")), p,
         device = ragg::agg_png, width = w, height = h,
...
# Globals at 1200 dpi (same sizes)
